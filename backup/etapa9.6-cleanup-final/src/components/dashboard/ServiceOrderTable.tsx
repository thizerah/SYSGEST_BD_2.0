...
export function ServiceOrderTable({ filteredOrders }: ServiceOrderTableProps) {
  const { serviceOrders } = useServiceOrdersData();
                  ...
                  {paginatedOrders.length === 0 ? (
                    ...
                    paginatedOrders.map(order => (
                      <TableRow key={order.codigo_os}>
                        ...
                        <TableCell>{formatDateTime(order.data_criacao)}</TableCell>
                        <TableCell>{formatDateTime(order.data_finalizacao)}</TableCell>
